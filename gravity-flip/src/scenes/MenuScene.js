class MenuScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.add.text(W / 2, H * 0.28, 'GRAVITY', {
      fontSize: '52px', fontFamily: 'Arial Black, Arial', color: '#ffffff'
    }).setOrigin(0.5);
    this.add.text(W / 2, H * 0.28 + 58, 'FLIP', {
      fontSize: '52px', fontFamily: 'Arial Black, Arial',
      color: '#00cfff', stroke: '#00cfff', strokeThickness: 1
    }).setOrigin(0.5);
    this.add.text(W / 2, H * 0.28 + 116, '탭하면 중력이 뒤집힌다', {
      fontSize: '13px', fontFamily: 'Arial', color: '#445566', letterSpacing: 2
    }).setOrigin(0.5);

    const best = parseInt(localStorage.getItem('gravity_flip_best') || '0');
    if (best > 0) {
